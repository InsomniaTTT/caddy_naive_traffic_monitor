...
func (h *Handler) UnmarshalCaddyfile(d *caddyfile.Dispenser) error {
	...
	for nesting := d.Nesting(); d.NextBlock(nesting); {
		...
		case "traffic_stats_path":
			args := d.RemainingArgs()
			if len(args) != 1 {
				return d.ArgErr()
			}
			h.TrafficStatsPath = args[0]

		case "traffic_stats_interval":
			args := d.RemainingArgs()
			if len(args) != 1 {
				return d.ArgErr()
			}
			interval, err := caddy.ParseDuration(args[0])
			if err != nil {
				return d.Errf("invalid traffic_stats_interval: %v", err)
			}
