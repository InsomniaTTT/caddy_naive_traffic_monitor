...
type Handler struct {
	...
	// Path to write per-user traffic statistics JSON file.
	// Default: "/var/log/caddy/traffic_by_user.json"
	TrafficStatsPath string `json:"traffic_stats_path,omitempty"`
...
func (h *Handler) Provision(ctx caddy.Context) error {
	...
	// Start per-user traffic stats writer (once, even if multiple handlers are provisioned).
	trafficStatsOnce.Do(func() {
		path := h.TrafficStatsPath
		if path == "" {
			path = "/var/log/caddy/traffic_by_user.json"
		}
		interval := time.Duration(h.TrafficStatsInterval)
		if interval <= 0 {
